...
const Groups = () => {
  ...
  const createPersonsTable = () => {
    db.transaction(txn => {
      txn.executeSql(
        'CREATE TABLE IF NOT EXISTS persons (id INTEGER PRIMARY KEY AUTOINCREMENT , name VARCHAR(100), surname VARCHAR(100), phone VARCHAR(15), company VARCHAR(100), email VARCHAR(50), address VARCHAR(200), group_id INTEGER, FOREIGN KEY (group_id) REFERENCES groups(id))',
        [],
        (sqlTxn, res) => console.log('kisi tablosu olusturuldu'),
        error => console.log('Hata', error.message),
      );
    });
  };
    ...
    <SafeAreaView style={{flex: 1}}>
      <View style={{padding: 10, flex: 1}}>
        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          <Button
            title={showDelete ? 'Done' : 'Edit'}
            onPress={() => setShowDelete(!showDelete)}
          />
          <Button onPress={() => handleAddNewItem()} title="Add List" />
        </View>
